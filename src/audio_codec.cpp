...
bool begin() {
  if (ready)
    return true;
  // Probe before configuring the codec/amp. Ordinary DevKits have no ES8388.
  if (!Wire.begin(config::codecSdaPin, config::codecSclPin))
    return false;
  Wire.setTimeOut(20);
  Wire.beginTransmission(0x10);
  if (Wire.endTransmission() != 0) {
    Wire.end();
    Serial.println("ES8388 absent: GPIO PWM audio remains available.");
    return false;
  }
  ...
  if (!codec.begin(settings)) {
    return fail("WARNING: ES8388 initialization failed; GPIO PWM remains available.");
  }
...
bool write(const int16_t *, size_t) { return false; }
